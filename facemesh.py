...
def preprocess_image(image, input_size, detector):
    ...
    if len(img_res.boxes) > 0:
        bbox = img_res.boxes[0]  # Get the first detected bounding box
        x1, y1, x2, y2 = bbox
        ...
        # Calculate margin to add around the detected face
        w = x2 - x1
        h = y2 - y1
        margin_h = int(0.25 * h)
        margin_w = int(0.25 * w)

        # Extend the crop area by the margin calculated
...
def process_output(landmarks, face_flag, preprocessed_frame, original_image, input_size, bbox):
    ...
    if face_flag > 0.5:
        ...
        # Check if preprocessed_frame is in the correct shape
        if len(preprocessed_frame.shape) != 4 or preprocessed_frame.shape[3] != 3:
            print("Preprocessed frame is not in the correct shape.")
            return original_image
